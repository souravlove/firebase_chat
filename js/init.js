		...
		anonymous.child(ke).on('child_added',snap_depth=>{ // ke => //device id
			...
			$.each(va,function(key,val){
				if ( typeof(val) != "object" ) {
			  		//console.log(key + " => " + val); //key is unique id val is msg info(object)
				} else  {
					//console.log(key + " => ");
...
var load_openhouse_chat = function() {
	chatType = 'openhouse';
	$('#chat_heading').html('Openhouse Admin Chat');
	openhouse.orderByChild('createdAt');
	$('ui.contacts').html('');
	openhouse_users.forEach(function(val,key){
		$('ui.contacts').append('<li>\
							<div onclick=showOHUserChat("'+val+'") class="d-flex bd-highlight">\
...
var load_anonymous_chat = function() {
	chatType = 'anonymous';
...
$('#send_msg').click(function(){
	...
	var user = globalUser;
	if(chatType == 'openhouse')
		sendOHUserMsg(user,msg);
	else
		sendAnonUserMsg(user,msg);
	$('#msg_box').val('');
});
...
var showAnonUserChat = function(user_id) {
	...
	ref.on('value',snap=>{
		$('#chatBox').html('');
		let obj = snap.val();
		$.each(obj,function(msg_root,msg){
			$.each(msg,function(key,val){
				if ( typeof(val) != "object" ) {
			  		console.log(key + " => " + val); //key = user,text,createdAt
				} else  {
					console.log(key + " => ");
					$.each(val,function(keyy,vall){	//only for user
						console.log(keyy,vall)	//keyy = _id,avatar
					});
				}
			});
...
sendOHUserMsg = function(user,msg) {
	const ref = openhouse.child(user);
	ref.push({
	    text: msg,
	    user: admin,
...
sendAnonUserMsg = function(user,msg) {
	const ref = anonymous.child(user);
	ref.push({
	    text: msg,
	    user: admin,
	    createdAt: firebase.database.ServerValue.TIMESTAMP,
